...
Given(/^the mock server for GET "([^"]*)" has a message response of$/) do |path, message|
  @mock_server.store_msg('get', path, message)
end
...
Given(/^I have a webresponse checker for "([^"]*)" and response "([^"]*)"$/) do |url, text|
  @webresponse = WebResponse.new('url' => url, 'text' => text)
end
...
Given(/^I have a webresponse "([^"]*)" checker for "([^"]*)" and response "([^"]*)" for "([^"]*)" $/) do |type, url, text, matcher|
  @webresponse = WebResponse.new('url' => url, 'type' => type, 'text' => text, 'matcher' => matcher)
end
